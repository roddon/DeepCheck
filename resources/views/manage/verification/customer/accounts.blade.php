...
<div class="text-center enter-info" id="account-error-area" style="display: none;">
    <p>Verification Failed, Please try again. <span><a href="{{ route('verification.customer.kyc', ['customer_id' => $customerId]) }}">Retry</a></span></p>
</div>
@endsection
...
<script>
    $('input[name=supplierAccountList]').click(function() {
        ...
        $.ajax({
            url: "{{route('verification.customer.add-account')}}",
            type: 'POST',
            data: {
    ...
    $(document).on('click', 'input[name=address]', function(){
        ...
        $('#account-area').hide();
        $.ajax({
            url: "{{route('verification.customer.add-address')}}",
            type: 'POST',
            data: {
                addressId: addressId,
            },
